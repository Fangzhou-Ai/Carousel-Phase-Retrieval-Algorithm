...
class CpraImpl
{
    public:
        ...
        virtual bool MergeAddData(std::complex<T>* flat_src, std::complex<T>* flat_dst, T alpha, size_t num) = 0;
        ...
        virtual bool Forward2D(std::complex<T>* flat_input, cudaStream_t stream) = 0;

        virtual bool Backward2D(std::complex<T>* flat_input, cudaStream_t stream) = 0;

        virtual bool Forward3D(std::complex<T>* flat_input, cudaStream_t stream) = 0;

        virtual bool Backward3D(std::complex<T>* flat_input, cudaStream_t stream) = 0;

        virtual bool SpaceConstraint(std::complex<T>* flat_src_data, T* flat_constr_data, size_t num, size_t batch_size, cudaStream_t stream) = 0;

        virtual bool RealDataConstraint(std::complex<T>* flat_src_data, T* flat_constr_data, size_t num, size_t batch_size, cudaStream_t stream) = 0;

        virtual bool ComplexDataConstraint(std::complex<T>* flat_src_data, std::complex<T>* flat_constr_data, size_t num, size_t batch_size, cudaStream_t stream) = 0;

        // Add src to dst
        // flat_dst = alpha * flat_src + flat_dst 
        virtual bool MergeAddData(std::complex<T>* flat_src, std::complex<T>* flat_dst, T alpha, size_t num, cudaStream_t stream) = 0;

        // flat_src = flat_src ./ norm
        virtual bool Normalization(std::complex<T>* flat_src, T norm, size_t num, cudaStream_t stream) = 0;
