...
static constexpr uint64_t N = M;
static constexpr uint64_t P = M * 9 / 16;
static constexpr uint64_t L = M;
...
void ShrinkWrap_CPRA_MKL_Sample(int epi, int iter)
{
    // 2D part test
    // Shrinkwrap
    CPRA::Cpra<float, CPRA::IMPL_TYPE::MKL> compute(M, N, L, BATCHSIZE_CPRA);
    //CPRA::Cpra<float, CPRA::IMPL_TYPE::MKL> memory(1, 1, 1, 1);
    ...
    */
    ...
    CPRA::Cpra<float, CPRA::IMPL_TYPE::MKL> compute_3D(M, N, L, 1);
...
{
    CPRA::Cpra<float, CPRA::IMPL_TYPE::MKL> compute(M, N, L, BATCHSIZE_CONV);
