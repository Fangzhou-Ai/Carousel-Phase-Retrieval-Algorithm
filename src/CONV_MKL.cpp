...
template<typename T>
std::pair<std::complex<T>*, T*> ShrinkWrap_CONV_MKL_Sample(int M, int N, int L, int BATCHSIZE_CONV, 
                                                            T* dataConstr, T* spaceConstr, 
                                                            int iter, T Beta = 0.9)
{
    // 2D part test
    // Shrinkwrap
    CPRA::Cpra<T, CPRA::IMPL_TYPE::MKL> compute(M, N, L, BATCHSIZE_CONV);
...
int main(int argc, const char* argv[])
{
    ...
    while(true)
    {
        ...
        auto results = ShrinkWrap_CONV_MKL_Sample<float>(parser.getM(),
                                                          parser.getN(),
                                                          parser.getL(),
                                                          n,
                                                          dataConstr,
